...
pub fn parseFile(allocator: std.mem.Allocator, path: []const u8) ParseResult {
    ...
    // Evaluate — push entry file to import_stack for circular import detection
    var evaluator = Evaluator.init(allocator);
    evaluator.base_dir = base_dir;
    evaluator.import_stack.append(allocator, real_path) catch {};
    const result: ParseResult = if (evaluator.evalDocument(doc)) |value|
        .{ .value = value }
    else |_| if (evaluator.collected_errors.items.len > 0)
        .{ .errors = evaluator.collected_errors.items }
    else
        singleError(allocator, evaluator.last_error orelse UzonError.runtimeError(allocator, "unknown evaluation error", 0, 0));
